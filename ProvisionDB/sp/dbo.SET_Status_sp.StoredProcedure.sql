USE [ProvisionDB]
GO
/****** Object:  StoredProcedure [dbo].[SET_Status_sp]    Script Date: 24.11.2021 18:29:16 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
    ...
    begin
        ...
        if @p_Status = 5 begin
            UPDATE [dbo].[OrderDates_tb] SET Delivered=NULL, Validated=NULL WHERE OrderID=@p_OrderID
            delete from [dbo].[Reviews_tb] where OrderID=@p_OrderID and Status in (8)
        end
        if @p_Status = 6 begin
            select @l_validated=Validated from [dbo].[OrderDates_tb] where OrderID=@p_OrderID
            if [dbo].[CHECK_IsEmpty_fn](@l_validated) = 1
